      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(0,212,170,0.15)', color: 'var(--accent-cyan)' }}>
              <DownIcon />
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>Download</div>
              <div className="text-2xl font-bold tabular-nums" style={{ color: 'var(--accent-cyan)' }}>{fmtBps(recvBps)}</div>
            </div>
          </div>
          <SparklineBlock data={recvHistory} color="var(--accent-cyan)" />
        </div>

        <div className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(251,146,60,0.15)', color: 'var(--accent-orange)' }}>
              <UpIcon />
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>Upload</div>
              <div className="text-2xl font-bold tabular-nums" style={{ color: 'var(--accent-orange)' }}>{fmtBps(sentBps)}</div>
            </div>
          </div>
          <SparklineBlock data={sentHistory} color="var(--accent-orange)" />
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <InsightCard
          title="Busiest Download Interface"
          name={busiestDownload?.name ?? '—'}
          primary={busiestDownload ? fmtBps(busiestDownload.recv_bps) : '—'}
          secondary={busiestDownload ? `${fmtTotalBytes(busiestDownload.bytes_recv)} received total` : 'Waiting for traffic'}
          accent="var(--accent-cyan)"
        />
        <InsightCard
          title="Busiest Upload Interface"
          name={busiestUpload?.name ?? '—'}
          primary={busiestUpload ? fmtBps(busiestUpload.sent_bps) : '—'}
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 360 }}>
          {nets.length > 0 ? [...nets].sort((a, b) => (b.recv_bps + b.sent_bps) - (a.recv_bps + a.sent_bps)).map((net, i) => (
            <div key={net.name} className="grid items-center px-5 py-3" style={{ gridTemplateColumns: '28% 18% 18% 18% 18%', borderBottom: i === nets.length - 1 ? 'none' : '1px solid rgba(255,255,255,0.03)' }}>
              ...
              <span className="text-xs tabular-nums font-mono" style={{ color: 'var(--accent-orange)' }}>{fmtBps(net.sent_bps)}</span>
              <span className="text-xs tabular-nums font-mono" style={{ color: 'var(--text-secondary)' }}>{fmtTotalBytes(net.bytes_recv)}</span>
              <span className="text-xs tabular-nums font-mono" style={{ color: 'var(--text-secondary)' }}>{fmtTotalBytes(net.bytes_sent)}</span>
            </div>
          )) : (
            <div className="flex items-center justify-center py-8"><span className="text-xs" style={{ color: 'var(--text-muted)' }}>Waiting for network data...</span></div>
          )}
        </div>
      </div>
    </div>
...
function InsightCard({ title, name, primary, secondary, accent }: { title: string; name: string; primary: string; secondary: string; accent: string }) {
  return (
    <div className="rounded-2xl p-5 flex flex-col gap-2" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
      <div className="text-[10px] uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{title}</div>
      <div className="text-lg font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{name}</div>
      <div className="text-2xl font-bold tabular-nums" style={{ color: accent }}>{primary}</div>
      <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>{secondary}</div>
    </div>
  )
}

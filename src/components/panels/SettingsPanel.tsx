...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="flex flex-col gap-3">
      <h2 className="text-[11px] font-bold uppercase tracking-widest px-1" style={{ color: 'var(--text-muted)' }}>{title}</h2>
    ...
    <div className="flex rounded-xl p-0.5 gap-0.5" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border)' }}>
      {options.map(opt => (
        <button key={String(opt.value)} type="button" onClick={() => onChange(opt.value)} className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-150"
          style={{
            background: value === opt.value ? 'linear-gradient(135deg, rgba(79,156,249,0.25) 0%, rgba(167,139,250,0.2) 100%)' : 'transparent',
...
function ThresholdSlider({ value, onChange, label, color }: { value: number; onChange: (v: number) => void; label: string; color: string }) {
  return (
...
const MENUBAR_OPTIONS: { label: string; value: MenubarMode }[] = [
  { label: 'CPU', value: 'cpu' },
  { label: 'Memory', value: 'memory' },
  { label: 'CPU + Memory', value: 'cpu_mem' },
    ...
    <div className="flex-1 overflow-y-auto p-6 animate-fade-slide">
      <div className="max-w-3xl mx-auto flex flex-col gap-6">
        <div className="flex flex-col gap-1 mb-2">
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Customize how ResourceScope looks and behaves</p>
        </div>
            ...
            <div className="flex rounded-xl p-0.5 gap-0.5" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border)' }}>
              {REFRESH_OPTIONS.map(opt => (
                <button key={opt.value} type="button" onClick={() => s.setRefreshInterval(opt.value)} className="px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all duration-150"
                  style={{
                    background: s.refreshIntervalMs === opt.value ? 'linear-gradient(135deg, rgba(79,156,249,0.25) 0%, rgba(167,139,250,0.2) 100%)' : 'transparent',
            ...
            <div className="flex rounded-xl p-0.5 gap-0.5" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border)' }}>
              {REFRESH_OPTIONS.map(opt => (
                <button key={opt.value} type="button" onClick={() => s.setMenubarRefreshInterval(opt.value)} className="px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all duration-150"
                  style={{
                    background: s.menubarRefreshIntervalMs === opt.value ? 'linear-gradient(135deg, rgba(79,156,249,0.25) 0%, rgba(167,139,250,0.2) 100%)' : 'transparent',
                    color: s.menubarRefreshIntervalMs === opt.value ? 'var(--text-primary)' : 'var(--text-muted)',
                    border: s.menubarRefreshIntervalMs === opt.value ? '1px solid rgba(79,156,249,0.3)' : '1px solid transparent',
                  }}>
                  {opt.label}
                </button>
              ))}
        ...
        <Section title="About">
          ...
          <Row label="Reset Settings" description="Restore all preferences to defaults" last>
            <button type="button" onClick={s.resetToDefaults} className="px-4 py-1.5 rounded-xl text-xs font-semibold transition-all duration-150"
              style={{ background: 'rgba(248,113,113,0.12)', color: 'var(--accent-red)', border: '1px solid rgba(248,113,113,0.2)' }}>
              Reset to Defaults
            </button>
          </Row>
        </Section>
